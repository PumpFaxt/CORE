...
Deno.test("Should not allow non admin to remove admin", async () => {
  ...
  await runtime.expectContractFunctionExecutionError(
    registry.write.removeAdmin([acc1.account.address], { account: acc2.account }),
  );
...
Deno.test("Should return correct list of admins after adding", async () => {
  const { owner, acc1, acc2, registry } = await runtime.loadFixture(deployFixture);
  ...
  const expectedAdmins = [owner.account.address, acc1.account.address, acc2.account.address];
...
Deno.test("Should not allow non-admin to register a forwarder", async () => {
  ...
  await runtime.expectContractFunctionExecutionError(
    registry.write.registerForwarder([acc1.account.address], { account: acc2.account }),
  );
})
...
Deno.test("Should not allow non-admin to remove a forwarder", async () => {
  ...
  await runtime.expectContractFunctionExecutionError(
    registry.write.removeForwarder([acc1.account.address], { account: acc2.account }),
  );
});
